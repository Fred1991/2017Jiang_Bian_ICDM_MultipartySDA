...
for i=1:nRun
    for j=1:nTrain
    [~, ~, ~, ~, f1Max1(i,j), accMax1(i,j), AUC1(i,j)] = phishingMPSDs(15,1,100+100*(j-1),leaderSize);
    [~, ~, ~, ~, f1Max2(i,j), accMax2(i,j), AUC2(i,j)] = phishingDS(0.01,100+100*(j-1));
    [~, ~, ~, ~, f1Max3(i,j), accMax3(i,j), AUC3(i,j)] = phishingDis(0.1,100+100*(j-1));
    end
