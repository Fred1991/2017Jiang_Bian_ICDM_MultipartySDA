...
multiaccS = cell(k,n);multiF1S = cell(k,n);multif1MaxS = zeros(k,n);multiaccMaxS = zeros(k,n);multiAUCS = zeros(k,n);multiTpS = cell(k,n);multiFpS = cell(k,n);
...
for j=1:2
    for i=1:n
    [multiaccS{j,i}, multiF1S{j,i}, multif1MaxS(j,i), multiaccMaxS(j,i), multiAUCS(j,i), multiTpS{j,i}, multiFpS{j,i}] = testpesudoS(lambda,lambdaGlasso,20+(j-1)*20,10);
